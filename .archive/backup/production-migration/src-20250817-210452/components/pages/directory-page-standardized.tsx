...
import { allFFLs } from '@/lib/ffl-directory-data'
...
// Convert FFL data to BusinessData format
const convertFFLToBusinessData = (ffl: any): BusinessData => ({
  businessName: ffl.businessName,
  businessType: ffl.businessType,
  description: ffl.description,
  address: ffl.address,
  phone: ffl.phone || 'Contact for information',
  website: ffl.website || '#',
  hours: ffl.hours || 'Contact for hours',
  tier: ffl.tier,
  specialties: ffl.specialties,
  isVerified: ffl.isVerified,
  isSponsored: ffl.isSponsored,
  imageUrl: ffl.imageUrl,
  slug: ffl.slug,
  category: ffl.category,
  featured: ffl.featured
})

// Existing directory listings (ranges, clubs, specialty venues)
const existingDirectoryListings: BusinessData[] = [
...
// Combine existing listings with comprehensive FFL directory
const directoryListings: BusinessData[] = [
  ...existingDirectoryListings,
  ...allFFLs.map(convertFFLToBusinessData)
]
