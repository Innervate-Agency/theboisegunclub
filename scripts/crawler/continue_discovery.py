#!/usr/bin/env python3
"""
Smart Website Discovery System - Continue Mode
===============================================
Continues discovery from where we left off, skipping already processed businesses.
"""
...
class SmartWebsiteDiscoveryContinue:
    ...
    def load_already_processed(self) -> set:
        ...
        if os.path.exists(self.discovered_file):
            ...
            processed_ids = set([d['business_id'] for d in discovered])
...
async def main():
    ...
    NEW_BATCH_SIZE = 50  # Process next 50 businesses
    
    print("🎯 TREASURE VALLEY WEBSITE DISCOVERY - CONTINUE MODE")
    print("=" * 55)
    ...
    try:
        ...
        print(f"🔍 Processing next {len(remaining_businesses)} businesses...")
        ...
        for i, business in enumerate(remaining_businesses, 1):
            print(f"[{i}/{len(remaining_businesses)}] Searching: {business['name']} ({business['city']})")
        ...
        if discoveries:
            ...
            print(f"📈 Success rate: {len(discoveries)/len(remaining_businesses)*100:.1f}%")
