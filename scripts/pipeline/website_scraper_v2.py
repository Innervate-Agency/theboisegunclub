#!/usr/bin/env python3
"""
Website Content Scraper v2 - With Deduplication
===============================================
Scrapes actual business websites to extract REAL data.
Skips websites already scraped to prevent duplicate work.
"""
    ...
    def scrape_business_website(self, business: dict) -> dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Mark in cache
                self.dedup_manager.mark_website_scraped(website, business)
...
def main():
    """Load discovered businesses and scrape their websites with deduplication"""
    
    print("🌐 WEBSITE CONTENT SCRAPER v2 (with deduplication)")
    print("=" * 60)
    ...
    if not discovered_files:
        ...
        print("   Run real_data_pipeline_v2.py first")
        return
    ...
    print(f"🏪 Found {len(businesses)} businesses from discovery")
    
    # Initialize deduplication and scraper
    dedup_manager = DeduplicationManager()
    scraper = BusinessWebsiteScraper()
    
    # Show cache status
    cache_stats = dedup_manager.get_cache_stats()
    if cache_stats['websites_scraped'] > 0:
        print(f"📊 SCRAPING CACHE:")
        print(f"   Websites already scraped: {cache_stats['websites_scraped']}")
        print()
    ...
    print(f"✅ {len(local_businesses)} LOCAL businesses after filtering")
    
    # Filter out already scraped websites
    new_businesses = dedup_manager.filter_new_businesses_for_scraping(local_businesses)
    
    if not new_businesses:
        print("✅ All business websites already scraped!")
        print("   Use existing scraped files or reset cache to re-scrape")
        return
    
    print(f"🌐 Will scrape {len(new_businesses)} new websites")
    print()
    
    # Scrape websites
    enriched_businesses = []
    
    for i, business in enumerate(new_businesses[:10], 1):  # Limit to 10 for testing
        print(f"[{i}/10] Processing...")
        enriched_business = scraper.scrape_business_website(business)
        enriched_businesses.append(enriched_business)
        
        # Be respectful with delays
        if i < len(new_businesses):
            time.sleep(3)
        print()
    ...
    print(f"🔄 Cache updated to prevent re-scraping")
