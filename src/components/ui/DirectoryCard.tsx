  ...
  return (
    <div className={cn(directoryCardVariants({ variant, size }), className)} {...props}>
      <div className="flex items-center gap-base mb-[var(--space-sm)]">
        {imageUrl && !imgError ? (
          <div className="relative h-14 w-14 rounded-card overflow-hidden bg-card-surface shadow-flat">
            <Image
              src={imageUrl}
              alt={name}
              fill
          ...
          <div className="h-14 w-14 rounded-card bg-sandy-ochre/10 flex items-center justify-center font-bold text-heading-sm text-dark-chocolate shadow-flat">
            {name.charAt(0)}
          </div>
        )}
