...
export default function AnimatedSplashCard({
  ...
  return (
    <div
      className={cn(
        "group relative overflow-hidden rounded-xs bg-card text-card-foreground p-lg shadow-flat transition-all duration-300 ease-out hover:shadow-elevated",
        className
      )}
      {...props}
    >
      ...
      <div className="absolute inset-0 opacity-0 transition-opacity duration-700 group-hover:opacity-30">
        <div className="absolute top-micro/4 left-1/4 w-1 h-1 bg-sandy-ochre rounded-full animate-ping animation-delay-100" />
        <div className="absolute top-xs/4 right-1/4 w-1 h-1 bg-rusty-orange rounded-full animate-ping animation-delay-300" />
        <div className="absolute top-micro/2 right-1/3 w-0.5 h-0.5 bg-walnut-stock rounded-full animate-ping animation-delay-500" />
      </div>
