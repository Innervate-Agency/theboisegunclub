...
const selectItemVariants = cva(
  ...
  "focus:bg-accent/80 focus:text-accent-foreground hover:bg-accent/80 hover:text-accent-foreground [&_svg:not([class*='text-'])]:text-muted-foreground relative flex w-full cursor-default items-center gap-xs rounded-sm outline-hidden select-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 transition-colors duration-150",
  {
    variants: {
      size: {
        sm: "py-xs pr-md pl-xs text-body-sm min-h-[28px]",
        default: "py-xs pr-lg pl-xs text-body-sm min-h-[32px]",
        lg: "py-sm pr-lg pl-sm text-body-base min-h-[40px]",
      },
    },
    defaultVariants: {
      size: "default",
...
>(({ className, children, icon, ...props }, ref) => {
  ...
  return (
    ...
    >
      ...
      <span className="absolute right-xs flex h-icon-sm w-icon-sm items-center justify-center">
...
function SelectSeparator({
  ...
  return (
    <SelectPrimitive.Separator
      ...
      className={cn("bg-border pointer-events-none -mx-micro my-micro h-px", className)}
      {...props}
    />
  )
}
...
function SelectScrollUpButton({
  ...
  return (
    <SelectPrimitive.ScrollUpButton
      ...
      className={cn(
        "flex cursor-default items-center justify-center py-micro",
        className
      )}
      {...props}
    >
